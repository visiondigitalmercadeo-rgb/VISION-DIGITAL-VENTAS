...
todos_los_vendedores = [
    v for v in db.list_vendedores(solo_activos=False) if not v.get("oculto_ventas_mes")
]
...
def _render_seccion(prefijo_columna, registros, key_prefix, upsert_fn, texto_boton, texto_exito):
    ...
    # Excluye del cálculo a cualquier vendedor marcado como "oculto en Ventas
    # por mes" (ver Administración de usuarios) — sus montos ya guardados no
    # se borran, pero no cuentan aquí mientras esté oculto, para que los
    # totales de arriba coincidan con lo que se ve en la tabla de abajo.
    ids_visibles = {v["id"] for v in todos_los_vendedores}
    registros = {vid: r for vid, r in registros.items() if vid in ids_visibles}
