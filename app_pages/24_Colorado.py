...
from config import (
    ...
    DISENO_ARCHIVO_MAX_BYTES_STORAGE, ESTADOS_COLORADO, PRODUCCION_ARCHIVOS_MAX,
)
...
def _subir_archivos_orden(archivos_subidos):
    ...
    if _usa_storage:
        return db.subir_archivos_storage_lista(
            "colorado", archivos_subidos, _archivo_max_bytes, PRODUCCION_ARCHIVOS_MAX,
        )
...
def _subir_boleta_pago(archivo_subido):
    ...
    if _usa_storage:
        return db.subir_archivo_storage("colorado_boletas", archivo_subido)
    ...
    with col:
        ...
        for p in items:
            with st.container(border=True):
                ...
                pdf_bytes = orden_produccion_pdf_bytes(p, linea="Colorado")
                st.download_button(
                    "📄 Orden de producción (PDF)", data=pdf_bytes,
                    file_name=f"orden_produccion_colorado_{pid}.pdf",
                    mime="application/pdf", use_container_width=True, key=f"colorado_pdf_{pid}",
                )
                    ...
                    if arch.get("storage_path"):
                        ...
                        if url_archivo:
                            st.link_button(
                                ...
                                use_container_width=True, key=f"colorado_file_{pid}_{i}",
                            )
                        else:
                    ...
                    if boleta_pago.get("storage_path"):
                        ...
                        if url_boleta:
                            st.link_button(
                                ...
                                use_container_width=True, key=f"colorado_boleta_{pid}",
                            )
                        else:
                            st.caption(
